...
L-Langは、LLMが要求から実行可能なバイナリを生成し、人間がソースコードを事前にレビューせずに利用する将来を想定しています。目指すのは、**要求と実行物の対応を保ち、必要なときにはTypeScriptで処理内容を確認できること**です。

処理の意味を表すSemantic Coreを共通基盤とし、実行用のWasmと人間が検査するTypeScript表現を結び付けます。正当な要求を外部データに紛れた命令から保護し、生成物とは独立した権限方針と実行時の制限を組み合わせることで、意図しない外部送信や許可外の書き込みなどを抑える構想です。

これは今後検証する研究上の主張です。現在の対応経路では検査済みの中間表現からWasmを生成し、任意の直接生成バイナリの安全性や、自然言語要求との完全な一致を保証するものではありません。設計の考え方と論文で確かめたい点は[メインコンセプト](./MAIN_CONCEPT.md)にまとめています。
