# Schema Evolution evaluation blocker

記録日: 2026-07-20

状態: **局所Blocker / 対応保留**

この文書は、Schema Evolutionのモデル出力を取得する前の事前確認で見つかった評価設計上の問題を記録する。

## 判定

現在の評価入力入力を`frozen`へ変更したり、live benchmarkを実行したりしてはならない。

このBlockerが停止するのは次の範囲である。

- Schema Evolutionのlive benchmark
- 現在の評価を根拠にした型付き2/3 Consensusの信頼性評価
...
- Freeze metadata: [`freeze.json`](./freeze.json)
- Frozen SHA-256: `c332a5c1586accc41935107cb043d5d3c1f976c490ad44bc6fbce96c4a4c4d61`
- `freeze.json`は`status: draft`
- 2026-07-20時点でSchema Evolutionのlive API実行結果は存在しない
- 同日時点のローカル確認は`bun run typecheck`成功、`bun test`が`50 pass / 0 fail`

`benchmark.json`と`freeze.json`は、このBlockerへの対応時にも現在の評価入力の証拠として保存する。
...
現在の4 Conceptは業務領域と型名こそ新しいが、resolved Oracleはいずれも主に次の論理骨格を使用する。

1. 肯定状態を`equals`で確認する
2. 除外timestampを`equals null`で確認する
3. 必須の役割を`present`で確認する

これは旧評価のActive CustomerおよびShippable Orderと同型である。新しい名前とスキーマだけで十分なheld-out性を持つか、また`any`や`not`を含む未知の論理構造へ一般化した証拠になるかは未確認である。

これはコンパイラの不具合ではなく、現在の評価から主張できる一般化範囲に対する留保である。

## なぜ全体Blockerではないか

確認された問題は、現在のConcept、Oracle、fixture境界の対応に局在する。

- `present`の実装は定義どおりに動作している
- 既存の型検査と回帰テストは成功している
- fail-closedとdraft時のAPI拒否は維持されている
- Static JudgmentはSchema Evolution manifest、Predicate IRの`present`、Schema Evolution Consensusへ依存しない

ただし、共通の`semantic.lock`をStatic Judgmentへ拡張する場合は、既存Predicate entryとreplayの後方互換性を回帰条件とする。

## 対応保留中の禁止事項

- `freeze.json`を`frozen`へ変更しない
- `bun run benchmark:schema-evolution`を実行しない
- 現在の評価入力をConsensus信頼性やPilot移行の成功証拠として扱わない
- 結果を改善する目的で既存`benchmark.json`を書き換えない
- このBenchmarkだけを理由に`present`の既存意味を変更しない
- 現在の評価入力を修正して再利用した結果を、新規held-out evidenceとして扱わない
...
4. 旧評価と異なる論理構造をheld-out評価へ含める必要があるかを決める
5. 現在の評価入力を保留記録として残し、独立した評価セットへ置き換えるかを決める

## 再開条件

次をすべて満たした後に、新しい独立入力のfreezeとlive評価を再開する。

- 上記の未決事項について期待意味が文書化されている
- resolved Oracleが空文字、空配列、`null`、`undefined`を含む必要な境界テストと一致する
- Oracle自身をfixtureで評価する自動検証が成功する
- 現在の評価入力を変更する場合は、それを新規held-out evidenceとして再利用しない方針が明記されている
...
- [`../../src/schema-evolution-benchmark.ts`](../../src/schema-evolution-benchmark.ts)
