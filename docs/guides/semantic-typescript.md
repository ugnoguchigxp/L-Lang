# Semantic TypeScript 利用ガイド

[全体案内](../../README.md) · [English](./semantic-typescript.en.md) · [入力と出力の対応](./language-routes.md)

TypeScript DSLのConceptから制限IRを解決し、通常のTypeScriptを生成する経路の詳細です。JSONC経路と併存します。以下の実装範囲・制限・lock・評価記録は、このSemantic TypeScript経路についての説明です。コマンドはリポジトリのルートで実行します。
...
以下は過去の評価記録であり、現在の品質Gateの実行結果ではありません。2026-07-25時点のローカル検証:
...
- このガイドの`semantic` CLIはTypeScript出力を扱う。Wasm出力・JSONC経路は[経路ガイド](./language-routes.md)を参照。LSPと汎用の独自runtimeは未提供
