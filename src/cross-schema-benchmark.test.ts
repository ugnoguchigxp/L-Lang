...
describe("cross-schema benchmark", () => {
  ...
  test("runs all 27 lockless trials without exposing hidden tests to the resolver", async () => {
    ...
    expect(saved.actualBody).toEqual(
      required(bodies.isBenchmarkActiveCustomer),
    );
    expect(saved.response.outputText).toContain('"outcome":"resolved"');
    expect(
      await readFile(resolve(runDirectory, "report.md"), "utf8"),
    ).toContain("Trial pass rate: 100.0%");
  }, 30_000);

  test("rejects malformed and escaping protocol inputs before resolver calls", async () => {
