...
describe("optional semantic review", () => {
  test(
    "keeps artifacts unchanged until Predicate and Static Judgment candidates are approved",
    async () => {
  ...
  test(
    "dispatches review build, diff, and approve through the CLI for both source kinds",
    async () => {
      const parent = resolve(workspaceRoot, ".semantic", "test-workspaces");
      await mkdir(parent, { recursive: true });
      ...
      try {
        ...
        expect(await exists(resolve(testRoot, "is-ready.generated.ts"))).toBeFalse();
        expect(await exists(resolve(testRoot, "semantic.lock"))).toBeFalse();
        const predicateDiff = await runSemanticCli(testRoot, ["diff", predicateId]);
        ...
        expect(await exists(resolve(testRoot, "is-ready.generated.ts"))).toBeTrue();
        ...
        expect(await exists(resolve(testRoot, "mike-is-cat.generated.ts"))).toBeFalse();
        ...
        expect(await exists(resolve(testRoot, "mike-is-cat.generated.ts"))).toBeTrue();
...
function renderPredicateSource(sourceDirectory: string): string {
  const conceptModule = modulePath(
    relative(sourceDirectory, resolve(workspaceRoot, "concepts", "active-customer")),
  );
...
function renderStaticJudgmentSource(sourceDirectory: string): string {
  const conceptModule = modulePath(
    relative(sourceDirectory, resolve(workspaceRoot, "examples/static-judgment/cat")),
  );
  const dslModule = modulePath(
    relative(sourceDirectory, resolve(workspaceRoot, "src", "dsl")),
  );
...
function candidateIdFrom(output: string): string {
  ...
  if (match?.[1] === undefined) throw new Error(`candidate id missing from:\n${output}`);
  return match[1];
}
