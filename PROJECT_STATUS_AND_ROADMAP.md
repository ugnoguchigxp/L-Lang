...
現在対応中なのは、機能範囲の拡張ではなく、次の問いに答えるための信頼性検証である。

> 未知のConceptと未知のSchema Evolutionに対し、3サンプル・型付き2/3コンセンサスが、誤解決を増やさず単発LLMより高い再現性を示すか。

このためのSchema Evolution v2は実装済みで、**独立した人間レビューと実Azure OpenAI実行を待っている**。
...
| Static Judgment | 未実装 | `staticValue`、`judgeStatic`、定数への意味的畳み込みは未着手 |
| Semantic Generation | Predicate限定で対応済み | `generatePredicate`からBoolean Predicate IRとTypeScriptを生成 |
| Static / Generated / Runtimeの段階分離 | 一部対応 | LLMはbuild/check時だけ使用し、生成物にLLM呼び出しを残さない。Static値の段階検査は未実装 |
...
| Semantic Lockfile | 実装済み | hash、model、IR、生成コードを固定し、APIなしのreplayに対応 |
...
| 仮説 | 評価 | 根拠または不足 |
| --- | --- | --- |
| 仮説1: 意味的判断を定数へ固定 | 未評価 | Static Judgment自体が未実装 |
...
| 全ローカル回帰 | 50 tests pass、0 fail | 実装とfixture評価の回帰なし |
...
2. Static Judgment
   - `staticValue`と`judgeStatic`
   - Runtime値をStatic Judgmentへ渡せない段階検査
   - 判断結果の定数化とlock/replay
3. Semantic Closure graph
...
最初の二つは、2026-07-20時点で`50 pass / 0 fail`である。最後のコマンドは現在、独立レビュー未完了のためAPIを呼ぶ前に停止する。
