...
`semantic explain`はPredicateとStatic Judgmentの両方について実装済みである。APIなし・読み取り専用でConcept、入力hash、lock済みresolution、provenance、生成物整合性を説明し、current / stale / unlocked / integrity-errorを区別する。実装範囲と回帰条件は[`SEMANTIC_EXPLAIN_IMPLEMENTATION_PLAN.md`](./SEMANTIC_EXPLAIN_IMPLEMENTATION_PLAN.md)を正とする。
